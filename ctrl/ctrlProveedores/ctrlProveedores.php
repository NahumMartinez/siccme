<?php
 include("../../models/clsProveedores/clsProveedores.php");
 ...
 $ve_IdEstProv		= $_POST['gp_IdEstProv'];
 $ve_IdTipoProv		= $_POST['gp_IdTipoProv'];
 $ve_FeNacimiento	= $_POST['gp_FeNacimiento'];
 $ve_IdPersona 		= $_POST['gp_IdPersona'];
 $ve_CodPersona		= $_POST['gp_CodPers'];
 $ve_IdRubro		= $_POST['gp_IdRubro'];
 $ve_operacion  	= $_POST['gp_Op'];
 
 // FIN de declaracion de variables ...
 
 // Instacia del Objeto Empleados
 $objectoProveedor = new clsProveedor($ve_CodProv, $ve_fCreacion, $ve_programa, $ve_usuario, $ve_NumIdentidad, $ve_NomName1, $ve_NomName2,									  $ve_Apelli1, $ve_Apelli2, $ve_Telefono, $ve_Celular, $ve_Email, $ve_RefDir, $ve_IdTipoProv, 									  $ve_IdEstProv, $ve_FeNacimiento, $ve_IdPersona, $ve_CodPersona, $ve_IdRubro);

 // Proceso de Buscar ********************************************************
    if($ve_operacion == "buscar")
    {
	 if($objectoProveedor->buscar())
	 {
		 $ve_CodProv 		= $objectoProveedor->get_cod_proveedor();
		 $ve_fCreacion  	= $objectoProveedor->get_f_creacion();
		 $ve_programa 		= $objectoProveedor->get_programa();
		 $ve_usuario 		= $objectoProveedor->get_nombre_usuario();
		 $ve_NumIdentidad  	= $objectoProveedor->get_num_identidad();
		 $ve_NomName1 		= $objectoProveedor->get_nom_1();
		 $ve_NomName2 		= $objectoProveedor->get_nom_2();
		 $ve_Apelli1  		= $objectoProveedor->get_ape_1();
		 $ve_Apelli2 		= $objectoProveedor->get_ape_2();
		 $ve_Telefono 		= $objectoProveedor->get_telefono();
		 $ve_Celular  		= $objectoProveedor->get_celular();
		 $ve_Email	 		= $objectoProveedor->get_email();
		 $ve_RefDir 		= $objectoProveedor->get_ref_dir();
		 $ve_FeNacimiento	= $objectoProveedor->get_f_nacimiento();
		 $ve_IdPersona 		= $objectoProveedor->get_id_persona();
		 $ve_CodPersona		= $objectoProveedor->get_cod_persona();
		 $ve_IdEstProv		= $objectoProveedor->get_id_est_proveedor();
		 $ve_IdTipoProv		= $objectoProveedor->get_id_tipo_proveedor();
		 $ve_IdRubro		= $objectoProveedor->get_id_rubro();
		 $ve_listo 			= 1;
		 
		 $result = array("find" => "ok" , "msg" => "Se han encontrado los Datos" , "valor1" => $ve_fCreacion , "valor2" => $ve_usuario, "valor3" => $ve_NumIdentidad, "valor4" => $ve_NomName1, "valor5" => $ve_NomName2, "valor6" => $ve_Apelli1, "valor7" => $ve_Apelli2, "valor8" => $ve_Telefono, "valor9" => $ve_Celular, "valor10" => $ve_Email, "valor11" => $ve_IdTipoProv, "valor12" => $ve_IdEstProv, "valor13" => $ve_CodPersona, "valor14" => $ve_RefDir, "valor15" => $ve_IdPersona, "valor16" => $ve_FeNacimiento, "valor17" => $ve_IdRubro);
